...
#fileName = 'WrongModelDoylePatientsDataset_noNoiseNoControl.pt'
fileName = 'WrongModelDoylePatientsDataset_noNoise.pt'

def createDataset(fileName, disableController):
    ...
    for p in range(nPatients):
        ...
        DoylePatient[0].paramsDict["controlParamsDict"]["q_o2"] = 0
        DoylePatient[1].paramsDict["controlParamsDict"]["q_o2"] = 0
        DoylePatient[0].paramsDict["controlParamsDict"]["q_H"] = 0.1
        DoylePatient[1].paramsDict["controlParamsDict"]["q_H"] = 0.1
        ...
        workRef = list()
        l = [-1, 1]
        workRef.append(DoylePatient[0].referenceValues["d_L"][None, :, :].repeat(batch_size, 1, 1) + l[np.random.randint(2)]*workRefOffset)
        workRef.append(DoylePatient[1].referenceValues["d_L"][None, :, :].repeat(batch_size, 1, 1) + l[np.random.randint(2)]*workRefOffset)
...
createDataset(fileName, False)
createDataset(fileName + 'NoControl', True)
